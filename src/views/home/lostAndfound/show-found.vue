          ...
          <div class="h-panel h-panel-no-border shadow">
            <div class="h-panel-bar">
              <div class="h-panel-title">
                <el-page-header title="返回" @back="goBack" content="招领启事详情"></el-page-header>
              </div>
            </div>
            <div class="h-panel-body">
              <Form readonly mode="twocolumn">
                ...
                <FormItem label="拾到地址：">{{lostItem.plcae}}</FormItem>
                <FormItem label="拾到时间：">{{lostItem.lostTime}}</FormItem>
                ...
                <template v-if="lostItem.contact == 1">
                  ...
                  <FormItem label="宿舍号：">{{ lostItem.dorm }}</FormItem>
                  <FormItem label="微信：">{{ lostItem.wechat }}</FormItem>
                </template>
                <template v-if="lostItem.contact == 2">
                  <FormItem label="认领站点：">{{ lostItem.contactType }}</FormItem>
                </template>
                <FormItem label="启事状态：">
...
export default {
  ...
  mounted() {
    let tempId = this.$route.query.foundId;
